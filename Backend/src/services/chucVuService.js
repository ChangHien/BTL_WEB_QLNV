...
export const createChucVu = async (payload = {}) => {
  ...
  if (exists) {
    return { error: "ma_chuc_vu đã tồn tại", status: 409 };
  }
  if (!isValidCode(maChucVu)) {

    return {
      error: "ma_chuc_vu phải gồm đúng 1 ký tự chữ hoặc số", 
      status: 400,
    };
  }
...
export const updateChucVu = async (maChucVu, updates) => {
  ...
  if (safeUpdates.ten_chuc_vu !== undefined) { 
    const nextValue = normalizeString(safeUpdates.ten_chuc_vu || ""); 
    if (!nextValue) {
      return { error: "ten_chuc_vu không hợp lệ", status: 400 }; 
    }
    safeUpdates.ten_chuc_vu = nextValue; 
  }
  await record.update(safeUpdates);
  const refreshed = await db.ChucVu.findByPk(record.ma_chuc_vu, { 
    raw: true,
  });
